...
require_once '../mailtemplates/forgotPassword.php';
...
if (!empty($_POST['username'])) {
  ...
  $checkUserResponse = $User->loginUser($username);
  ...
  switch ($checkUserResponse['response']) {
    ...
    $User->updateTokens($token, $code, $checkUserResponse['data'][0]['entry_id']);

    $messageBody = forgotPasswordTemplate($checkUserResponse['data'][0]['username'], $code, $_ENV['APP_URL'], $token);
